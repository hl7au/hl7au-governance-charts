...
def _chip_metrics(model: Model, theme: Theme, chip_width: float,
                  area_height: float) -> tuple[float, float, int]:
    """Chip height, gap and line count — uniform across the whole diagram.

    Chips grow to fit the longest label, then give that back if a column would
    otherwise run past the bottom of its card.
    """
    lines = max((len(wrap(p, chip_width - 46, theme.fs_chip))
                 for wg in model.work_groups for p in wg.projects), default=1)
    height = max(CHIP_HEIGHT, lines * LINE_HEIGHT + 34)
    gap = CHIP_GAP
...
def render(model: Model, theme: Theme | None = None,
    ...
    chip_h, gap, _ = _chip_metrics(model, t, chip_w, area_h)
    ...
    for i, wg in enumerate(model.work_groups):
        ...
        for j, project in enumerate(wg.projects):
            ...
            lines = wrap(project, chip_w - 46, t.fs_chip)
            start = y + chip_h / 2 - (len(lines) - 1) * LINE_HEIGHT / 2 + 7
            for k, line in enumerate(lines):
                c.text(chip_x + chip_w / 2 + 2, start + k * LINE_HEIGHT, line,
